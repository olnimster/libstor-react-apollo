...
function TransitionDown(props: string) {
...
const CreateAvatar: ({avatar, publicName}: propsType) => (JSX.Element) = ({avatar, publicName}: propsType) => {
    ...
    const [uploadFile, {loading, error}] = useMutation(UPLOAD_FILE, {
        onCompleted: () => {
            setTransition((): any => TransitionDown);
            setOpen(true);
        },
    });
